            ...
            <div class="panel-body">
                ...
                <div id="hbreadcrumb" class="pull-right m-t-lg">
                    <ol class="hbreadcrumb breadcrumb">
                        <li><a href="{{ url('inventario.editar') }}">Listadso de inventarios de proyecto</a></li>
                        <li class="active">
        ...
        <div class="col-lg-12">
            <div class="hpanel">
                ...
                <div class="panel-body">
                    ...
                    <div class="row">
                        <div class="col-md-12 col-sm-12 col-xs-12">
                            <div id="collapse_ver_inventario" class="collapse">
                                <button type="button" class="btn btn-primary" id="btn_ver_inventario">Ver Inventario</button>
                                <hr style="border-color: #FFF;">
                            </div>
                        </div>
                    </div>
                    <!--div id="collapse_inventario" class="collapse"-->
                    <div class="row" id="container_tabla_inventario">
                        <div class="col-md-12 col-sm-12 col-xs-12">
                            <hr>
                            <h4 id="texto_inmobiliaria_proyecto" class="text-info"></h4>
                            ...
                            <table id="example1" class="footable table table-stripped toggle-arrow-tiny" data-page-size="10" data-filter=#filter>
                                <thead>
                                    <tr>
                                        ...
                                        <th>Tiempo configuración en hora</th>
                                        <th>Cantidad</th>
                                        <th>Total</th>
                                        <th>Proyecto</th>
                                        <th>Producto</th>
                                        ...
                                        <th data-hide="all">Proyecto</th>
                                        <th data-hide="all">Producto</th>
                                        <th data-hide="all">Acción</th>
                                    </tr>
                                </thead>
                                <tbody id="datos_tabla_inventario">
                                </tbody>
                                <tfoot>
                                    <tr>
                                        <td colspan="5">
...
$(window).load(function(){
    //$('#container_tabla_inventario').css("display", "none");
});
</script>
...
$(document).ready(function(){
    $('#inmobiliarias').on('change', function(e){
        ...
        if(e.target.value == ''){
            $('#proyectos').empty().append('<option value="">-- Seleccione un proyecto --</option>');
            $('#texto_inmobiliaria_proyecto').empty();
            $('#datos_tabla_inventario').empty().append().trigger('footable_redraw');
            $('#collapse_ver_inventario').collapse('hide');
        } else {
            $('#collapse_ver_inventario').collapse('show');
            //$('#container_tabla_inventario').removeAttr('style');
    ...
    // Mostrar tabla inventario por proyecto
    $('#btn_ver_inventario').on('click', function(e){

    //$('#btn_ver_inventario').unbind('click').click(function(e){
        //e.preventDefault();

        var proyecto_id = $('#proyectos').find(":selected").val();
        var nombre_inmobiliaria = $('#inmobiliarias').find(":selected").text();
        var nombre_proyecto     = $('#proyectos').find(":selected").text();
        var nombre_proyecto_append = '';
        var nombre_producto_append = '';

        $('#texto_inmobiliaria_proyecto').empty().append(nombre_inmobiliaria+' | '+nombre_proyecto);

        $.get('listar_inventario_proyecto/'+proyecto_id, function(data){
            console.log(data);
            $('#datos_tabla_inventario').empty();
            $.each(data['inventarios'], function(index, value){
                $.each(data['proyectos'], function(i,v){
                    if(value.proyecto_id === v.id){
                        nombre_proyecto_append = v.nombre;
                    }
                });
                $.each(data['productos'], function(i,v){
                    if(value.producto_id === v.id){
                        nombre_producto_append = v.producto;
                    }
                });
                $('#datos_tabla_inventario').append(
                '<tr>'+
                    '<td bgcolor="#ffb606" style="color: #FFF;">Editar</td>'+
                    '<td>'+value.tiempo_instalacion_hora.toFixed(2)+'</td>'+
                    '<td>'+value.tiempo_configuracion_hora.toFixed(2)+'</td>'+
                    '<td>'+value.cantidad+'</td>'+
                    '<td>'+value.total.toFixed(2)+'</td>'+
                    '<td>'+nombre_proyecto_append+'</td>'+
                    '<td>'+nombre_producto_append+'</td>'+
                    '<td>'+
                        '<span class="pie">'+
                            '<div class="form-group">'+
                                '<input type="text" class="form-control" id="instalacion'+value.id+'" name="instalacion'+value.id+'" value="'+value.tiempo_instalacion_hora.toFixed(2)+'" readonly>'+
                            '</div>'+
                        '</span>'+
                    '</td>'+
                    '<td>'+
                        '<span class="pie">'+
                            '<div class="form-group">'+
                                '<input type="text" class="form-control" id="configuracion'+value.id+'" name="configuracion'+value.id+'" value="'+value.tiempo_configuracion_hora.toFixed(2)+'" readonly>'+
                            '</div>'+
                        '</span>'+
                    '</td>'+
                    '<td>'+
                        '<span class="pie">'+
                            '<div class="form-group">'+
                                '<input type="number" class="form-control" id="cantidad'+value.id+'" name="cantidad'+value.id+'" value="'+value.cantidad+'" min="1">'+
                            '</div>'+
                        '</span>'+
                    '</td>'+
                    '<td>'+
                        '<span class="pie">'+
                            '<div class="form-group">'+
                                '<input type="text" class="form-control" id="total'+value.id+'" name="total'+value.id+'" value="'+value.total.toFixed(2)+'" readonly>'+
                            '</div>'+
                        '</span>'+
                    '</td>'+
                    '<td>'+
                        '<span class="pie">'+
                            '<div class="form-group">'+
                                '<select class="form-control" id="proyecto'+value.id+'" name="proyecto'+value.id+'">'+
                                    '<option value="'+value.proyecto_id+'">Actual: '+nombre_proyecto_append+'</option>'+
                                '</select>'+
                            '</div>'+
                        '</span>'+
                    '</td>'+
                    '<td>'+
                        '<span class="pie">'+
                            '<div class="form-group">'+
                                '<select class="form-control" id="producto'+value.id+'" name="producto'+value.id+'">'+
                                    '<option value="'+value.producto_id+'">Actual: '+nombre_producto_append+'</option>'+
                                '</select>'+
                            '</div>'+
                        '</span>'+
                    '</td>'+
                    '<td>'+
                        '<div class="form-group">'+
                            '<button type="button" style="margin-right: 5px;" class="btn btn-sm btn-info" onclick="editar_inventario('+value.id+')">Guardar</button>'+
                            '<button type="button" class="btn btn-sm btn-danger" onclick="eliminar_inventario('+value.id+')">Eliminar</button>'+
                        '</div>'+
                    '</td>'+
                '</tr>'
                ).trigger('footable_redraw');
                var option_select = value.id;
                $.each(data['productos'], function(i,v){
                    $("#producto"+option_select+"").append('<option value="'+v.id+'">'+v.producto+'</option>');
                });
                $.each(data['proyectos'], function(i,v){
                    $("#proyecto"+option_select+"").append('<option value="'+v.id+'">'+v.nombre+'</option>');
                });
            });
            //console.log(typeof(data_producto));
            //console.log(Object.keys(data_producto).length);
            $(function(){
                $('#example1').footable();
            });
        });
...
function editar_inventario(id){
    ...
    var tiempo_instalacion_hora     = $("#instalacion"+id+"").val();
    var tiempo_configuracion_hora   = $("#configuracion"+id+"").val();
    var cantidad                    = $.trim($("#cantidad"+id+"").val());
    var total                       = $("#total"+id+"").val();
    var proyecto_id                 = $("#proyecto"+id+"").find(":selected").val();
    var producto_id                 = $("#producto"+id+"").find(":selected").val();
    ...
    if(!$.isNumeric(cantidad)){
        ...
        toastr.error('Error - Debe ingresar un número');
    } else if(cantidad <= 0){
        toastr.options = {
            "debug": false,
            "newestOnTop": false,
            ...
            success: function(data){
                ...
                if(data.respuesta === 0){
                    ...
                    '</tr>'
                        ...
                        var option_select_edit = value.id;
                        $.each(data['productos'], function(i,v){
                            $("#producto"+option_select_edit+"").append('<option value="'+v.id+'">'+v.producto+'</option>');
                        });
                        $.each(data['proyectos'], function(i,v){
                            $("#proyecto"+option_select_edit+"").append('<option value="'+v.id+'">'+v.nombre+'</option>');
                        });
                    });
                    $(function(){
                        $('#example1').footable();
                    ...
                    toastr.error('Error - en actualizar');
                }//Fin else
            },
            error: function(xhr){
