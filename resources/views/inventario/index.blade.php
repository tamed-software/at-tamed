            ...
            <div class="panel-body">
                ...
                <div id="hbreadcrumb" class="pull-right m-t-lg">
                    <ol class="hbreadcrumb breadcrumb">
                        <li><a href="{{ url('inventario') }}">Crear inventario de proyecto</a></li>
                        <li class="active">
                ...
                <h2 class="font-light m-b-xs">
                    Crear Inventario de Proyectos
                </h2>
                <small>Crear inventario</small>
            </div>
        </div>
    </div>
        ...
        <div class="col-lg-12">
            <div class="hpanel">
                <div class="panel-heading">
                    ...
                    Agregar inventario de proyecto
                </div>
                <div class="panel-body">
                    <div class="row">
                    ...
                    <div id="collapseCrearInventario" class="collapse">
                    <button type="button" class="btn btn-primary" id="btnVerProductos">Ver productos</button>
                    <div id="collapseBtnInventario" class="collapse">
                    <form id="form">
                    <div class="row">
                        <div class="col-md-12 col-sm-12 col-xs-12">
                        	<hr>
                            ...
                            <hr style="border-color: #FFF;">
                            <table cellpadding="1" cellspacing="1" class="table table-bordered table-striped table-hover" id="tabla_inventario">
                                <thead>
                                    <tr>
                                ...
                                <tbody>
                                    ...
                                    <tr id="datos_inventario">
                                        <td style="display: none;">{{ $producto->id }}</td>
                                        <td>
                                            <center>
                                                <div class="checkbox checkbox-info">
                                                    <input type="checkbox" id="seleccion_producto" name="seleccion_producto[]" value="{{ $producto->id }}">
                                                    <label for="seleccion_producto"></label>
                                                </div>
                                            </center>
                                        </td>
                    ...
                    <div class="row">
                        <div class="col-md-12 col-sm-12 col-xs-12">
                            <hr style="border-color: #FFF;">
                            <button type="submit" class="btn btn-success btn-block btn-lg" disabled="true" id="btnAgregarProductoInventario">Agregar Productos a Inventario</button>
                        </div>
                    </div>
                    </div>
                    </form>
...
<!-- MODAL VER CONTRATO -->
<div class="modal fade hmodal-info" id="" tabindex="-1" role="dialog"  aria-hidden="true">
    <div class="modal-dialog modal-lg3">
        <div class="modal-content">
            <div class="color-line"></div>
            <div class="modal-header">
                <h4 class="modal-title">Datos del contrato</h4>
                <small class="font-bold">Formulario para agregar producto.</small>
            </div>
            ...
            <div class="modal-footer">
                <button type="button" class="btn btn-info" id="btnAgregarProducto">Guardar</button>
...
$(document).ready(function(){
    ...
    $('#inmobiliarias').on('change', function(e){
        ...
        if(e.target.value == ''){
            $('#collapseCrearInventario').collapse('hide');
            $('#collapseBtnInventario').collapse('hide');
            $('#proyectos').empty().append('<option value=""></option>');
            $('#btnAgregarProductoInventario').prop('disabled', true);
        } else {
	        ...
	        $('#collapseCrearInventario').collapse('show');
	        //$('#collapseBtnInventario').collapse('hide');
	        $('#btnAgregarProductoInventario').prop('disabled', false);
        }
    });

    //Verificar la selección de un proyecto
    $('#btnVerProductos').on('click', function(e){
    	e.preventDefault();
    	...
    	if(proyecto_id == ''){
            ...
            toastr.error('Error - No se ha seleccionado un proyecto');
            $('#collapseBtnInventario').collapse('hide');
    	} else {
    		$('#collapseBtnInventario').collapse('show');
    	}
    });

    //Verificar cantidad de selecion de productos
    $('#btnAgregarProductoInventario').on('click', function(e){
    	e.preventDefault();
    	if($('[name="seleccion_producto[]"]:checked').length <= 0){
    		toastr.options = {
                "debug": false,
                "newestOnTop": false,
                "positionClass": "toast-top-center",
            ...
            toastr.error('Error - Seleccione al menos 1 producto');
    	} else {
    		var arrayProductos = [];
    		var arrayCantidadCero = [];
            ...
            $.ajax({
                ...
                success: function(data){
                    ...
                    if(data == 1){
                        ...
                        toastr.error('Error - El proyecto ya tiene asignado un inventario de productos');
                        table.$('input[type="checkbox"]:checked').each(function(){
                        ...
                        /*
                        $('#form')[0].reset();
                        $('#tabla_inventario').dataTable().fnClearTable();
                        $.get('/listado_tabla', function(data){

                        });
                        */
                        //$("#tabla_inventario").load("http://18.236.97.163:8001/inventario #tabla_inventario");
                        //$('#collapseCrearInventario').collapse('hide');
                        //$('#collapseBtnInventario').collapse('hide');
                        //$('#inmobiliarias').empty().append('<option value="">-- Seleccione una Inmobiliaria --</option>');
                        //$('#proyectos').empty().append('<option value=""></option>');
                        toastr.options = {
                            "debug": false,
                            "newestOnTop": false,
                        ...
                        toastr.success('Inventario agregado');
                        /*
                        setTimeout(function(){
                            location.reload('http://18.236.97.163:8001/inventario')
                        }, 1000);
                        */
                    }
                },
