...
<html>
  ...
  <h3 style="text-align: center;">{{ $parqueGarciaHuerta }}</h3>
  <hr>
  ...
  <table style="width: 100%;">
    ...
    @foreach($pdf_parque_garcia_huerta as $data)
    <tr>
      ...
      <td style="width: 100px;"><small>{{ $data->fecha_agendamiento }}</small></td>
    </tr>
    @endforeach
	</table>

  <hr>
  <h3 style="text-align: center;">{{ $alercesJazmines }}</h3>
  <hr>
  ...
  <table style="width: 100%;">
    ...
    @foreach($pdf_alerces_jazmines as $data)
    <tr>
      <td><small>{{ $data->nombre_estado }}</small></td>
      <td><small>{{ $data->vivienda }}</small></td>
      <td><small>{{ $data->nombre }}</small></td>
